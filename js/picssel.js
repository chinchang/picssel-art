...
undo = function() {
  var clear, i, last_point, last_state, len, p, pos;
  ...
  if (last_state.new_color === 'clear') {
    ...
    for (pos = i = 0, len = pixels.length; i < len; pos = ++i) {
      p = pixels[pos];
...
onMouseDown = function(e) {
  var color, cx, cy, i, j, len, len1, p, pixel_current_color, pos, rgb;
  ...
  if (e.shiftKey) {
    ...
    for (pos = j = 0, len1 = pixels.length; j < len1; pos = ++j) {
      p = pixels[pos];
      if (p.x === cx && p.y === cy) {
        pixels.splice(pos, 1);
        break;
...
addState = function(x, y, old_color, new_color) {
  ...
  if (states.length > undo_size) {
    return states.splice(0, 1);
  }
};
...
getRGB = function(color) {
  ...
  if (color.indexOf('#' === -1)) {
    return (ref = color.match(/rgba?\((\d*),(\d*),(\d*),?\d*\)/)) != null ? ref.slice(1).map(function(o) {
      return parseInt(o);
    }) : void 0;
  }
};
